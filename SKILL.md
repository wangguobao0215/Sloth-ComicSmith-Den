...
## Stage 5c: Audio Synthesis（音频合成）

**Goal**: Automatically generate a complete audio layer — BGM, character voices, foley/ambience, and styled subtitles — turning a silent slideshow into an emotionally immersive short film.

**Why this matters**: Sound is not decoration; it is half the story. A shot of rain without rain sounds is just a blue-gray filter. A character's bitter smile without the tremor in their voice is just a drawing.

### Step 1: Generate Audio Plan

Run `scripts/generate_audio_plan.py` to analyze `storyboard.json` and output `audio_plan.json`:

```bash
python scripts/generate_audio_plan.py my_project --bgm-source auto
```

This automatically:
- **Matches BGM** to each shot's `mood` + `scene_function` using the emotion-music catalog (solo piano for melancholic, orchestral for climactic, ambient for weary)
- **Assigns TTS voices** to each character based on age/gender/personality (young female weary → soft hesitant voice; middle-age male kind → warm fatherly voice)
- **Maps foley events** from `foley_key_moments` to precise timestamps
- **Generates subtitle timing** with exact start/end per dialogue line, distinguishing `spoken` / `thought` / `narration`

**Output**: `audio_plan.json` containing:
- `bgm_cues`: consolidated music segments with genre, BPM, key, intensity, and AI generation prompts
- `voice_cues`: per-line TTS config with speaker, text, emotion, timestamp
- `foley_cues`: sound effects with precise timing
- `subtitle_entries`: styled subtitle data for SRT/ASS export

### Step 2: Generate Subtitles

```bash
python scripts/srt_generator.py my_project --style anime
```

Outputs both `subtitles.srt` (universal) and `subtitles.ass` (styled with anime presets: white text, black outline, speaker names). Styles available: `anime` / `dialogue` / `minimal`.

### Step 3: Synthesize Character Voices（可选）

```bash
# Free local option (no API key)
python scripts/synthesize_voice.py my_project --provider edge

# High-quality paid option
python scripts/synthesize_voice.py my_project --provider elevenlabs --api-key YOUR_KEY
```

- **edge-tts** (free): Uses Microsoft Edge TTS voices. Good for Chinese. No cost. Requires `pip install edge-tts`.
- **ElevenLabs** (paid): Cinematic-quality multilingual voices. Emotional control. ~$0.03/line.

Voice clips save to `assets/audio/voice/{shot_id}_{speaker}.mp3`.

### Step 4: Mix Audio

```bash
# With a pre-existing BGM file
python scripts/mix_audio.py my_project --bgm assets/audio/bgm/my_music.mp3

# Or auto-mode (shows BGM prompts, you generate music with Suno/Udio, then re-run)
python scripts/mix_audio.py my_project --auto-bgm
```

Mixes BGM + voice clips + foley/ambience into `assets/audio/mixed_audio.mp3` using FFmpeg `amix`.

### Step 5: Compile Final Video with Audio & Subtitles

```bash
python scripts/compile_video.py my_project \
  --transition fade \
  --audio assets/audio/mixed_audio.mp3 \
  --subtitles-style anime \
  --output output_final.mp4
```

**Honest limitation**: FFmpeg subtitle burn-in requires libass support. If your FFmpeg lacks it, subtitles will be output as external `.srt` files — load them in VLC/MPV, or use a player that supports ASS styling.
...
├── audio_plan.json                 # Generated by generate_audio_plan.py
├── cost_estimate.json              # Generated by cost_estimator.py
├── subtitles.srt
├── subtitles.ass                   # Styled anime subtitle format
├── raw_script.md
...
│       ├── foley/
│       ├── ambience/
│       └── mixed_audio.mp3         # Final mixed audio track
